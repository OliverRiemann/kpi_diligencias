...
ui <- bs4DashPage(
  title = "Enterprise Demo",            # Browser tab title
  header = bs4DashNavbar(title = "Enterprise Demo", skin = "dark", status = "primary",
                         border = TRUE, sidebarIcon = "bars", controlbarIcon = NULL,
                         rightUi = tags$li(class="nav-item dropdown",
                                           tags$a(href="#", icon("user"), class="nav-link"))),
  sidebar = bs4DashSidebar(
    skin = "light", status = "primary", title = "Demo Menu",
    bs4SidebarMenu(
      bs4SidebarMenuItem("Dashboard", tabName = "dashboard", icon = icon("tachometer-alt"), selected = TRUE)
  ...
  body = bs4DashBody(
    bs4TabItems(
      ...
      bs4TabItem(tabName = "dashboard",
                 ...
                 fluidRow(
                   valueBox("15,234", "Total Sales", icon = icon("dollar-sign"), color = "success", width = 3),
                   valueBox("423", "New Orders", icon = icon("shopping-cart"), color = "info", width = 3),
                   valueBox("32%", "Growth Rate", icon = icon("chart-line"), color = "primary", width = 3),
                   valueBox("7.4", "Customer Rating", icon = icon("star"), color = "warning", width = 3)
                 ),
                 # Row 2: Two charts side by side
                 fluidRow(
                   box(title = "Sales by Group", status = "primary", solidHeader = TRUE, width = 6, collapsible = TRUE,
                       selectInput("group_var", "Group by:", choices = c("Category", "Region")),
                       plotOutput("plot_sales", height = "250px")
                   ),
                   box(title = "Revenue Over Time", status = "primary", solidHeader = TRUE, width = 6, collapsible = TRUE,
                       plotOutput("plot_trend", height = "250px")  # placeholder for a line chart
                   )
                 ),
                 # Row 3: Full-width chart (e.g., regional performance)
                 fluidRow(
                   box(title = "Regional Performance", status = "primary", solidHeader = TRUE, width = 12, collapsible = TRUE,
...
server <- function(input, output, session) {
  ...
  sales_data <- data.frame(Category = c("Product A", "Product B", "Product C"),
                           Sales = c(120, 90, 65))
  ...
  region_data <- data.frame(Region = c("North", "South", "East", "West"),
